...
type DbPatientInsert = Omit<DbPatient, 'created_at' | 'updated_at'>;
type DbPrescriptionInsert = Omit<DbPrescription, 'created_at'>;
type DbReceiptInsert = Omit<DbReceipt, 'created_at'>;

// Helper functions to convert between DB and App types
function dbPatientToPatient(db: DbPatient): Patient {
  return {
    id: db.id,
...
function patientToDbPatient(patient: Patient): DbPatientInsert {
  return {
    id: patient.id,
...
function dbPrescriptionToPrescription(db: DbPrescription): Prescription {
  return {
    id: db.id,
...
function prescriptionToDbPrescription(p: Prescription): DbPrescriptionInsert {
  return {
    id: p.id,
...
function dbReceiptToReceipt(db: DbReceipt): VisitReceipt {
  return {
    id: db.id,
    patientId: db.patient_id,
    patientName: db.patient_name,
...
function receiptToDbReceipt(r: VisitReceipt): DbReceiptInsert {
  return {
    id: r.id,
